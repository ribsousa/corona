...
export default {
  data: () => ({
    ...
    selectedCountry: {
      ...
      deaths: 4638,
      recovered: 79371,
      updated_at: '2020-05-28T23:32:33.000Z'
    }
  }),
  ...
  computed: {
    location () {
      return this.$store.state.location.items
    },
  ...
  methods: {
    ...
    getCountryByCode (code) {
      console.log('chamou getCountryByCode')
      const codes = JSON.parse(JSON.stringify(isoCodes))
    ...
    loadLocation () {
      console.log('chamou method')
      this.$store.dispatch('loadLocation')
        .then(() => {
          const location = this.$store.state.location.items
